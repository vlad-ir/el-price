...
(даже если такого цвета нет в 1С, но фотография с цветом есть на платформе).
...
foreach ($sel_arr as $key => $value) {
    ...
    if (!in_array($artikul, $artikul_arr)) {
        ...
        $artikul_file = strtolower(translit($artikul));

        // Каталог с фотографиями
        $filename = './prodimg/' . $artikul_file;
        $image_PDF = []; // Массив с именами файлов фотографий для PDF-документа
    
        if(file_exists($filename)){ //Если каталог с фотографиями существует
            $color_photo = find_all_files($filename); // Читаем все фото для артикула
            foreach ($color_photo as $color_key => $color_value) {
                if(mb_strrpos($color_value, '_1.jpg')){ // Выбираем по одной первой фотографии для каждого цвета
                    $image_PDF[] = $color_value;
                }
            }
        }

            // Количество фтографий для разных цветов
            $col_photo = count($image_PDF);
