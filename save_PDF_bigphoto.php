<?php 
/* ============================================================================
Формирует PDF-файл с выбранными артикулами и фотографиями  
(если такой цвет есть в 1С и фото с цветом загружено на платформу).
Максимум по 2 фотографии для выбранного артикула. Группировка по артикулам.
...
foreach ($sel_arr as $key => $value) {
    ...
    if (!in_array($artikul, $artikul_arr)) {
        ...
        $filename1 = PhotoFileName($artikul, $color).'_1.jpg';
        $filename2 = PhotoFileName($artikul, $color).'_2.jpg';

        //$image_PDF = (file_exists($filename)) ? '<img src="'.$filename.'">' : '<img src="./images/PDF_no_photo.jpg">';
        $image_PDF1 = (file_exists($filename1)) ? $filename1 : 0;
        $image_PDF2 = (file_exists($filename2)) ? $filename2 : 0;

        $col_photo_1 = ($image_PDF1) ? 1 : 0 ;
        $col_photo_2 = ($image_PDF2) ? 1 : 0 ;

        $col_photo = $col_photo_1 + $col_photo_2;
			...
			if ($col_photo > 0) { //Если есть фотографии
		       ...
		       $html_PDF_txt .= '<span style="background-color: rgb(255, 0, 0); color: rgb(255, 255, 255);">Цена с НДС, '.$currency_ID.': '.$value['nds_price'].'</span>';
		        ...
		        if ($col_photo == 2) { //Если две фотки
		                // add a page
		                $pdf->AddPage('L', 'A4');

		                // Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false)
		                $pdf->Image($image_PDF1, 0, 0, '', 210, '', '', '', false, 150, '', false, false, 0, false, false, false);
		                $pdf->Image($image_PDF2, 148.5, 0, '', 210, '', '', '', false, 150, '', false, false, 0, false, false, false);
